...
# language, implementation, specs, focused, and perturb. Doctrine lives in
# the skills; this script adds none.
...
grep -q '^- specs:[[:space:]]*[^[:space:]]' "$rig" || missing="$missing specs"
grep -q '^- focused:[[:space:]]*[^[:space:]]' "$rig" || missing="$missing focused"
grep -q '^- perturb:[[:space:]]*[^[:space:]]' "$rig" || missing="$missing perturb"
