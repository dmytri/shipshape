...
case "$payload" in
  *"STOP. Captain's notes"*)
    echo "Shipshape dispatch guard: the dispatch to $target carries the Captain-notes banner. Captain content crosses no firewall. Remove it and re-dispatch. (Article 2: context firewall.)" >&2
    exit 2
    ;;
esac
