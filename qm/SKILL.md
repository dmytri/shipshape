...
- Derive verification work from durable artifacts, not chat.
...
2. Read durable inputs for the current target.
3. Write or update missing executable coverage.
4. Run focused verification.
5. For one failing implementation target, load Crew and become Crew, or dispatch Crew.
6. After Crew returns and verification passes, invoke `/bosun` or dispatch Bosun as a subagent. Assume Bosun duties yourself only if the harness cannot dispatch a separate role — and say so explicitly if you do.
7. If requirements are unclear, load Captain with the blocker context; after Captain resolves it, the user must clear before QM runs again.
