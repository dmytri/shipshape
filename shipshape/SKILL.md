...
Captain is the human-facing discovery role. Quartermaster must not inherit Captain/human discovery chat. Captain tells the user to clear the session or start a fresh session before `/qm`; Quartermaster enforces that context firewall and refuses if Captain context is visible.
...
Use after implementation verification passes. Clean stale artifacts, rerun verification, commit locally, then load Captain for outbound decisions.
