...
- Crew MAY expose a narrow verification seam when required for the assigned failing target. Per the "Code exposes verification seams" Article, keep product behaviour out of constructors, global state, static initialization, service locators, test-only branches, and harness-only paths, and do not perform broad testability refactors, dependency rewrites, or architecture cleanup beyond the failing target.
