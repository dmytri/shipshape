...
- **Watchbill rule.** QM's own discovery is static: undefined and unimplemented steps. An implemented scenario becomes a target only through `watchbill.json` or a live perturbation, per the Watchbill policy. Valid `watchbill.json` selects and orders targets per that policy.
...
7. Collect the directed targets: discovery findings, watchbill selections, and live perturbations. When work-in-flight spec edits touch a scenario whose steps are all implemented and no watchbill selects it, report the watchbill omission to Captain; do not infer the target. If no directed target stands, report deck clean, scoped to what ran: static discovery and directed focused runs, not an enumeration of the failing set. Note any unrelated operator work left in the tree; next role Captain.
