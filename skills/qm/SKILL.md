...
- **Watchbill rule.** Worklist comes from undefined, unimplemented, or failing verification targets. Valid `watchbill.json` selects and orders a subset of that worklist per Article 12; process all watches in order unless verification, product intent, environment, or tooling blocks.
...
6. Run the `discover` command from `RIGGING.md` to identify undefined, unimplemented, or failing targets. If `RIGGING.md` defines no `discover` command, infer one from the project stack, fall back to per-scenario `focused` runs across all spec files, or block to Captain as a configuration blocker.
