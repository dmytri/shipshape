...
- **Plank-only dispatch on a promoted skeleton.** A seam a `@captain` skeleton described carried no plank, because no executable step bound its behaviour and a plank names a step definition's pattern, per the Planking agreement. Making that step executable creates the pattern. When such a target passes against existing production code, the seam its steps exercise is now plankable and unplanked: dispatch Crew with a plank-only target, carrying the scenario reference and the unplanked seam as the observed evidence. This is the same route a custody foul takes, and it is the moment the harbour finding closes.
