...
- **Plank-only dispatch on a promoted skeleton.** A seam a `@captain` skeleton described carries a provisional plank naming that skeleton's step line, per the Planking agreement. Making the step executable creates the step definition's pattern, so the provisional plank now matches neither legal form and owes normalization. The provisional plank names the seam, so no search is needed: dispatch Crew with a plank-only target carrying the scenario reference and that plank's location as the observed evidence. Where the derived plank-form check is live, the drift reddens it and arrives as an ordinary failing target instead. This is the same route a custody foul takes.
