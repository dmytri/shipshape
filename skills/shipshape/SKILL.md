...
- A plank string is one of two forms, and both are decidable: a current step-definition pattern, or the step line of a `@captain` scenario. The second form is provisional. A seam a `@captain` skeleton describes has no step definition yet, so no pattern exists to name; the plank carries the skeleton's step line and holds the seam. The provisional form is what keeps the seam findable: the plank is the durable link from a seam to the behaviour it carries, and a link that lives only in a role's report dies with that context. A promoted scenario whose seam was never planked reaches a fresh role as a behaviour with no code pointer, and the seam is reimplemented and left as dead code.
- A provisional plank liquidates itself. When Captain promotes the scenario and QM makes its step executable, the pattern comes into existence and the provisional plank matches neither form: it reddens as plank drift and normalizes to the pattern, per the QM skill's plank-only dispatch. The provisional form expires exactly when the `@captain` tag does.
...
- Cross-reference each plank against the two legal forms by exact string match: the step-definition patterns from `step-usage`, and the step lines of current `@captain` scenarios. A plank matching neither is stale or malformed: the step definition was deleted or renamed, or a provisional plank outlived its skeleton's promotion and owes normalization to the new pattern. Stale and malformed planks are corrected, never trusted.
