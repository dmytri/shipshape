...
9. **Every production seam is planked.** Planks are the behaviour-bearing production code required by Gherkin step contracts. Every production seam MUST have at least one `@planks(...)` annotation, and MUST NOT contain behaviour outside its related step contracts. A plank names a step definition's pattern, so a seam waits for its plank until an executable step binds its behaviour: a seam a `@captain` skeleton describes carries the skeleton and a reported harbour finding, and its plank lands when QM makes that step executable, per the QM skill's plank-only dispatch. The Planking agreement carries the annotation mechanics; the "Current design only" Article carries the judgment on unplanked seams.
...
- The plank string is exactly one of the project's step-definition patterns. That makes plank form decidable and checkable: a plank matching no current pattern is stale or malformed, per Judging.
...
- Cross-reference each plank's pattern against the `step-usage` command output by exact string match. A plank matching no current pattern is stale or malformed: the step definition was deleted, renamed, or the plank was never a pattern. Stale and malformed planks are corrected, never trusted.
