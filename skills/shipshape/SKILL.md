...
The wake MAY also carry the voyage run record, at the `runrecord` path from `RIGGING.md`. After a fresh green verification run, the running role appends one line: one JSON object with exactly the keys `targets`, `command`, `result`, and `hash`, where `hash` is the deck-state hash. Write the line in exactly this shape, key names verbatim:
