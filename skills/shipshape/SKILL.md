...
- A scenario asserts one lane, and the lane is legible from the scenario itself. Product behaviour is the default lane and carries no tag: what the product does, in the user's own terms. A `@contract` scenario attests the product's mechanical shape: the schema a response conforms to, the status codes it serves, an invariant a checker discharges. A `@conformance` scenario attests the project's own method: watchbill shape, perturbation quiescence, plank form, the verification-conformance rule set.
- One scenario, one lane. A behaviour and the contract its output satisfies are two scenarios, not one scenario asserting both. A reader of a mixed scenario cannot tell which half is the product's intent and which half is attestation, and a failing count cannot say which lane broke. Split the seam: one scenario proves the resource is created, another attests the response conforms to its schema.
- The lanes carry different custody, which is why the distinction is worth a tag. A product scenario is the user's: Captain removes it only with the user, per the "Current design only" Article. A `@contract` scenario attests the shipped artifact and outlives the code beneath it. A `@conformance` scenario attests how the project is built rather than what it ships: Shipwright derives it, and it leaves with the method it guards.
...
- The plank carries the step definition's pattern string verbatim, led by that definition's own keyword, such as `@planks("When I ask for the next low tide after {string}")`. The pattern is the durable contract; a scenario's concrete example values are not. A plank that copies a concrete step line goes stale with every data edit, and it cannot join against `step-usage`, which reports patterns. A step definition binds `Given`, `When`, or `Then`, so a scenario's `And` or `But` reaches the plank already resolved. Normalize `And` and `But` to the inherited `Given`, `When`, or `Then` when reading a scenario to find its binding.
- A plank string is exactly one of the project's current step-definition patterns. That one form makes plank form decidable: a plank matching no current pattern is stale or malformed, per Judging.
- `@planks-provisional("<scenario reference>")` is the provisional plank, and it is a distinct annotation, never a second form of `@planks`. A seam a `@captain` skeleton describes has no step definition yet, so no pattern exists to name. The provisional plank names the skeleton's scenario instead, in the repo-root-relative `<spec>.feature:<Scenario Name>` reference form the watchbill and the `focused` command already use. It holds the seam and announces itself: a reader sees at once that the plank is provisional and which scenario it waits on.
- The provisional plank is what keeps the seam findable. A plank is the durable link from a seam to the behaviour it carries, and a link living only in a role's report dies with that context. A promoted scenario whose seam was never planked reaches a fresh role as a behaviour with no pointer to the code that already implements it, so the seam is reimplemented and the original is left as dead code.
- A provisional plank liquidates itself, and promotion is the trigger. Captain promotes by removing the `@captain` tag, so a `@planks-provisional` naming a scenario that no longer carries `@captain` is red. QM finds the seam by searching for the provisional annotation that names the promoted scenario, with no seam hunt, and dispatches Crew to replace it with `@planks(...)` carrying the step-definition pattern QM has just authored, per the QM skill's plank-only dispatch.
...
- List planks with the `plank-inventory` command from `RIGGING.md` when defined; it inventories `@planks` and `@planks-provisional` alike. Also scan for bare tokens across the implementation paths, because a token outside a docblock on a declaration is a malformed plank the docblock reader cannot see.
- Plank form and plank placement are executable checks, not human-read judgment. Where the stack has no docblock or AST reader, they ride the derived verification-conformance rule set as rules, per the Shipwright skill's Methodology checks, each proven red by a plant at adoption. A token scan finds a malformed plank; only the rule proves the form, and a check no rule set carries is a check no role runs.
- The trace runs seam to plank to step definition to scenarios, and the runner already owns the last hop. `step-usage` reports each step-definition pattern with the scenarios that bind it, so a plank naming the pattern reaches every scenario through usage. A plank copying a concrete step line would store a second copy of a join the runner derives for free, and that copy drifts.
- Cross-reference each `@planks` string against the step-definition patterns from `step-usage` by exact string match. A plank matching no current pattern is stale or malformed: its step definition was deleted or renamed. Stale and malformed planks are corrected, never trusted.
- Cross-reference each `@planks-provisional` reference against the `@captain` scenarios in the specs. A provisional plank naming a scenario that carries `@captain` conforms and waits. A provisional plank naming a promoted scenario is red and owes its pattern; a provisional plank naming no current scenario is stale.
...
A role that runs in the foreground needs no narration rule. Its commands, its runs, and its edits reach the human as they happen, so the channel itself is the human's view. Silence is created by isolation: dispatching a role into an isolated context hides its work, and the human watches a blank deck for as long as that role runs.

A role that dispatches into isolation owes that visibility back. Dispatch into an isolated context only where the dispatched role's progress reaches the human: directly, where the runtime surfaces it, or through an active seat that narrates it. Where neither route holds, run the role in the foreground channel instead.

The narrating seat surfaces plain text while dispatched work is in flight, at least once every two minutes: what is running, what it has produced, what it is waiting on. A status display, a task list, and a raw transcript dump are not narration; narration is prose a human reads. It carries the dispatched role's observable signals, such as streamed verification output, tier transitions, and touched files, never its context. This holds the on-deck rule: internal roles stay terse in their artifacts, and only their observable signals are narrated. A live signal surfaces a wrong turn while a human can still act. An announcer tone is welcome.

Narration is not a wait. The narrating seat reports what it can already see, and completion arrives as the dispatched role's report, per Hand-off custody. A seat that wakes to ask whether the work has finished has built the busy-wait that rule forbids.

A runtime that spawns roles SHOULD carry this surfacing as machinery, since a nested role cannot narrate for a role it did not dispatch, and only the runtime sees every seat.
...
A role's turn ends only in its final report. A role never ends its turn waiting: not on a background command, a notification, a timer, or another agent. Work in flight at report time is either consumed before the report or named in it as a blocker. A verification run whose output the role has not read is not evidence and counts as never run.
