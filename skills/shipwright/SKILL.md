...
7. Scan for policy violations and seams with behaviour outside the step contracts in the saved step-text mapping:
   ...
   - **Slow seam:** a scenario whose per-scenario duration from the boundary check's weather record, step 12, is an outlier against the rest of its tier. A first harbour reads a prior session's weather when the wake carries it; otherwise this category populates when step 12 runs. Report the scenario and its duration as a Captain finding; whether to split, optimize, or accept it is a product judgment, not a harbour edit.
