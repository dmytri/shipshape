...
- Scantlings: detect existing scantling files such as an OpenAPI document, a JSON Schema, a GraphQL schema, or a proof contract file, as defined in the scenario-writing agreement. Propose adoption in a `@captain` scenario that references the scantling and asserts the seam conforms, or, for a proof contract, attests a clean discharge. Do not author a project-owned scantling; Captain writes it.
