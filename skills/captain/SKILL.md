...
- Keep `CAPTAIN.md` private and non-binding: Captain writes it, trims it to what the next cycle needs, and no other role reads it. Notes custody is Captain's: a notes edit that is the only work in flight is Captain's to commit directly, never left to block a clean-deck gate; a notes edit riding a voyage's role-advanced work is staged content-blind by Boatswain with the custody commit, per the Boatswain skill.
