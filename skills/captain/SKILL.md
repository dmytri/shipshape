...
- **Dependencies:** Dependency selection is a product decision. When a dependency is needed, research options with the user, confirm the selection, and write it into `RIGGING.md` under `## Dependencies`. Write only the dependency name. Do not pin a version unless the scenario or the dependency policy requires it. Installation is fitting out, per the Rigging read contract: every dependency, the rigging's own and the product's alike, is installed and upgraded by Shipwright at fitting out or at harbour, or by Captain on the greenfield fast path, and none routes through Crew; the installing role records each installed dependency under `## Dependencies` in the same pass. When Crew blocks on a confirmed dependency that is not installed, confirm the selection, record it, and route the install to Shipwright.
...
4. Dispatch Boatswain for harbour custody: the post-implementation job over the harbour-scoped edits, per the Dispatch contract, loading Boatswain in place only when operating without subagents.
