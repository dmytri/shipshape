...
- Recheck selection is a lookup, not a judgment. Staged hunks select the recheck; the watchbill is not a recheck selector. Per staged hunk, exactly one row applies:
  - The caller's hand-off carries a fresh focused green covering the hunk: inherit it as commit evidence; run nothing.
