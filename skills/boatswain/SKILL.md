...
- Strike a spent watchbill, per the Watchbill policy: when the caller's hand-off reports the watchbill spent and the recheck is green, remove `watchbill.json` and stage the removal with the custody commit. A watchbill with entries no report verifies is not spent: leave it and name it in the report for Captain.
