...
- Custody is evidence-or-rerun, never both. A staged role-advanced hunk whose fresh focused green the caller's hand-off carries keeps that run as its commit evidence: do not rerun it. Rerun by trace only for a staged executable hunk with no carried evidence, or where the diff contradicts the hand-off; follow the seam's planks to its focused scenario set per the Planking agreement's selection rule and run that set. A traced scenario whose steps are undefined has never been verified: it is QM work waiting on the watchbill, not a failing recheck, and custody proceeds without it. A staged hunk no plank traces, support edits, deletions, and configuration, is proved by static discovery plus the derived `typecheck` and `lint` gates, which redden on a broken load or import. When no staged hunk touches executable surface, no recheck runs. When a recheck runs and unrelated operator work remains unstaged, stash the unstaged changes, run the recheck against the staged state, and restore the stash.
- Strike a spent watchbill, per the Watchbill policy: when the caller's hand-off reports the watchbill spent, remove `watchbill.json` and stage the removal with the custody commit. The spent report is the strike's whole evidence: the strike is a non-executable deletion and orders no run of its own. A watchbill with entries no report verifies is not spent: leave it and name it in the report for Captain.
