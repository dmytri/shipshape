...
- **Dead seams.** Production seams with no `@planks(...)` annotation are dead. Flag them as Captain blockers and mark with a `@shipwright` docblock tag for Shipwright to remove during the next harbour session:
  ...
  Boatswain does not delete production code. Before flagging any code, check for `@captain` and `@shipwright` scenarios that describe the module. Code described by a `@captain` scenario is protected while it awaits Captain review. Code planked to a `@shipwright` scenario is already condemned; leave the existing mark and add nothing.
