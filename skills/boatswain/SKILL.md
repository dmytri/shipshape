...
- MAY read `CAPTAIN.md` for note hygiene only; MUST NOT edit it.
...
4. Read `CAPTAIN.md` for note hygiene when present. Flag bloated or outdated notes as blockers.
...
- When a path listed under `scantlings` in `RIGGING.md` has no referencing scenario: report it to Captain per the Scantling agreement.
...
- When a live `PERTURBATION` statement stands in the working tree: name it in the report. Execution enforces liveness per the Perturbation policy; the harbour full regression is the net.
...
- When `CAPTAIN.md` is bloated: flag it for trimming. Judge by size against its carry-only banner; the content is Captain's.
...
- Reverify by trace, per the Planking agreement's selection rule. The voyage's earlier work keeps its fresh focused runs as commit evidence; the recheck covers Boatswain's own staged hunks. Follow each touched seam's planks to its focused scenario set and run it. A staged hunk no plank traces, support edits, deletions, and configuration, is proved by static discovery plus the derived `typecheck` and `lint` gates, which redden on a broken load or import. When no staged hunk touches executable surface, no recheck runs. When a recheck runs and unrelated operator work remains unstaged, stash the unstaged changes, run the recheck against the staged state, and restore the stash.
