...
- Be ruthless about current design: no stale specs, orphaned steps, orphaned tests, dead fixtures, stale implementation, or historical tombstones.
...
- **Remove unreachable production code.** The implementation directory is disposable and exists only to satisfy current specs/tests; code that no current scenario, test, or step exercises, a path orphaned after a behaviour was retired or refactored, is dead and MUST be removed when unambiguous. Detect it by following from the current verification surface (grep for unreferenced exports/functions; where practical, a coverage pass over the BDD run flags implementation lines no test reaches). When it is unclear whether a path is truly unreachable vs. covering live behaviour the tests merely under-exercise, leave it and raise a Captain blocker rather than risk removing a real contract.
...
- Production code with no `Shipshape implements` trace link is dead. Remove it. A `@shipwright` link counts as a trace link and protects the code; never remove `@shipwright`-traced code. Run focused verification after removal; if suite stays green, the code was dead. If suite goes red, revert and flag to Captain.
...
- **Spec is the contract.** When inspecting traced code, verify it implements exactly the behaviour the scenario describes. Any deviation, extra behaviour, missing behaviour, different behaviour, side effects, additional state changes, unlisted outputs, is a spec gap or dead code. Flag to Captain. Captain decides: spec it, or hand back to Bosun for deletion.
