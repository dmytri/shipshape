...
5. Map covered code to step text. For each covered production file, find which step definitions import or reference it. Read those step definitions for Gherkin step bindings (e.g. `Given(`, `When(`, `Then(` calls in Cucumber-JS, decorators in Cucumber-JVM, or equivalent in the project's language). The string argument is the canonical Gherkin step text. Save this mapping for step 9. If multiple step definitions reference the same file, the file may carry Planks for multiple steps.
