...
12. Run the full test suite across all tiers as a boundary check, each tier by its tier tag from `RIGGING.md`. Compose the tier tag with the `@captain` exclusion, for example `--tags "@sandbox and not @captain"`. The harbour session reconstitutes the codebase; verify nothing broke. If a tier cannot run because credentials are absent, report it as a skipped target per the verification policy.
