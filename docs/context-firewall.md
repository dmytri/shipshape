...
2. If the current session contains any Captain/human discovery discussion, product decisions, clarifications, or ad hoc instructions that are not committed to repository artifacts, refuse to continue.
...
5. Use only those durable artifacts plus verification output QM runs itself.
...
```text
Context firewall passed. I do not see Captain/human discovery context in this session. I will use only AGENTS.md, HANDOVER.md, committed specs, tests, referenced assets, and verification output I run in this session.
```
